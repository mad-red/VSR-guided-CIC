...
patience = 0
if opt.sample_rl:
    saved_data = torch.load('saved_model/coco_cap/%s_best.pth' % opt.exp_name)
...
for e in range(start_epoch, start_epoch+100):
    if not opt.sample_rl:
        # Training with cross-entropy
        model.train()
        running_loss = .0
        running_loss_gate = .0
        with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(iter(dataloader_train))) as pbar:
            for it, (detections, _, ctrl_det_seqs, ctrl_det_gts, _, _, captions) in enumerate(iter(dataloader_train)):
        ...
        with tqdm(desc='Epoch %d - baseline' % e, unit='it', total=len(iter(dataloader_train))) as pbar:
            with torch.no_grad():
                for it, (detections, _, ctrl_det_seqs, ctrl_det_gts, ctrl_det_seqs_test, _, caps_gt) in enumerate(iter(dataloader_train)):
    ...
    if not os.path.exists('saved_model/coco_cap/'):
        os.makedirs('saved_model/coco_cap/')
    ...
    if val_cider >= best_cider:
        ...
        torch.save(saved_data, 'saved_model/coco_cap/%s_%s.pth' % (opt.exp_name, best_srt))
    else:
        patience += 1
        saved_data['patience'] = patience
    torch.save(saved_data, 'saved_model/coco_cap/%s_last.pth' % opt.exp_name)
